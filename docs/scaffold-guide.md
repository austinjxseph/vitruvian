...
- Any CDN scripts (e.g. GSAP) are loaded before the Svelte bundle via `scripts.php`.
...
│       ├── scripts.php           # Any CDN scripts + components.js
...
Request → index.php → Kirby router → site/templates/{name}.php
  ...
  → snippet("scripts")    ← any CDN scripts + components.js
  → closes </html>
```

### `site/snippets/head.php`
- Outputs `<!DOCTYPE html>`, `<head>` and opens `<body class="body">`
...
Loaded at the **very end** of every template (just before `</html>`). At minimum:
```html
<script src="/assets/js/components.js"></script>
```

Add any CDN libraries your site uses before `components.js`. Order matters: CDN libs load first, then `components.js` (registers and mounts Svelte components).
...
The `svelte-ready` event can be listened to in any custom script if you need to run logic after all components have mounted (e.g. measuring final page height).
...
The only required script is `components.js`. Add any CDN libraries your design needs **before** it:

```html
<!-- Example: if using GSAP -->
<script src="https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js"></script>

<!-- Always last: Svelte component bundle -->
<script src="/assets/js/components.js"></script>
```

If you have custom vanilla JS (e.g. scroll behaviour, animations), put it in `assets/js/script.js` and load it before `components.js`:
...
`components.js` dispatches `svelte-ready` on `window` when all components have mounted. Listen for this in `script.js` if you need to measure the final page layout after hydration.

---

## 12. Content Files & Folder Conventions
...
## 13. Kirby Panel Configuration

### Accessing the Panel
`/panel` route. First-run creates an admin user (see section 22).
...
## 14. Deployment: Dockerfile for Railway
...
## 15. Railway-Specific Configuration
...
## 16. Kirby Config for Reverse Proxies
...
## 18. Step-by-Step: Scaffold a New Site
...
- Wrap content in `<div class="page"><main class="main">…</main></div>`
...
## 21. Adding a New Svelte Component
...
## 22. Kirby Panel First-Run Setup
